function varargout = rr_export_fbx_xodr(varargin)
% RR_EXPORT_FBX_XODR Export a RoadRunner scene to FBX and XODR.
%
%   [status, exportPath, logPath] = rr_export_fbx_xodr('param', value, ...)
%
...
%   'preserveSource' - Logical: preserve original scene files (default true)
    ...
    if isempty(scenePath) || ~exist(scenePath, 'file') == 2
        error('rr_export_fbx_xodr:SceneNotFound', ...
    ...
    if isempty(rrPath) || ~exist(rrPath, 'file') == 2
        error('rr_export_fbx_xodr:RoadRunnerNotFound', ...
            'RoadRunner executable not found.');
    end

    % Resolve working directory.
    if isempty(workDir) || ~exist(workDir, 'dir') == 7
        workDir = fullfile(tempdir, 'rr_export_fbx');
    end
    ...
    if isempty(outputDir)
        ...
        outputDir = fullfile(workDir, ['export_fbx_', timestamp, '_', uid]);
    end
    ...
    logFileName = fullfile(logDir, ['export_fbx_', timestamp, '.log']);
    logFid = fopen(logFileName, 'w');

    log(logFid, '=== FBX/XODR Export Log ===');
    ...
    % Build export command for FBX.
    exportCmd = sprintf( ...
        'rrExport(''%s'', ''%s'', ''fbx'', ''%s'');', ...
        rrPath, scenePath, outputDir);

    log(logFid, 'Running FBX export command...');
    ...
    % Also export XODR if FBX succeeded or as fallback.
    if status == 0
        xodrOutputDir = fullfile(outputDir, 'xodr');
        if ~exist(xodrOutputDir, 'dir') == 7
            mkdir(xodrOutputDir);
        end
        xodrCmd = sprintf( ...
            'rrExport(''%s'', ''%s'', ''xodr'', ''%s'');', ...
            rrPath, scenePath, xodrOutputDir);
        log(logFid, 'Running XODR export...');
        try
            [xodrStatus, ~] = system(xodrCmd);
        catch ME
            xodrStatus = 1;
            log(logFid, 'XODR export ERROR: %s', ME.message);
        end
        if xodrStatus ~= 0
            log(logFid, 'XODR export failed with code %d', xodrStatus);
        end
    end
    ...
    if status ~= 0
        error('rr_export_fbx_xodr:ExportFailed', ...
