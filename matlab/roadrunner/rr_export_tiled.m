function varargout = rr_export_tiled(varargin)
% RR_EXPORT_TILED Export a RoadRunner scene to Tiled map format.
%
%   [status, exportPath, logPath] = rr_export_tiled('param', value, ...)
%
...
%   'preserveSource' - Logical: preserve original scene files (default true)
    ...
    if isempty(scenePath) || ~exist(scenePath, 'file') == 2
        error('rr_export_tiled:SceneNotFound', ...
    ...
    if isempty(rrPath) || ~exist(rrPath, 'file') == 2
        error('rr_export_tiled:RoadRunnerNotFound', ...
            'RoadRunner executable not found.');
    end

    % Resolve working directory.
    if isempty(workDir) || ~exist(workDir, 'dir') == 7
        workDir = fullfile(tempdir, 'rr_export_tiled');
    end
    ...
    if isempty(outputDir)
        ...
        outputDir = fullfile(workDir, ['export_tiled_', timestamp, '_', uid]);
    end
    ...
    logFileName = fullfile(logDir, ['export_tiled_', timestamp, '.log']);
    logFid = fopen(logFileName, 'w');

    log(logFid, '=== Tiled Export Log ===');
    ...
    % Build export command for Tiled.
    exportCmd = sprintf( ...
        'rrExport(''%s'', ''%s'', ''tiled'', ''%s'');', ...
        rrPath, scenePath, outputDir);

    log(logFid, 'Running Tiled export command...');
    ...
    if status ~= 0
        error('rr_export_tiled:ExportFailed', ...
