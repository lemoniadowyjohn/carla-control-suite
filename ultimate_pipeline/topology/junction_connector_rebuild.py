...
def _write_rebuild_geometry(
    *,
    ...
    allow_straight_chord_fallback: bool,
) -> Optional[str]:
...
def rebuild_displaced_junction_connectors_on_root(
    ...
    *,
    ...
    max_abs_curvature: float = 1.0,
    max_examples: int = 25,
    allow_straight_chord_fallback: bool = False,
) -> Dict[str, Any]:
    roads_by_id: Dict[str, ET.Element] = {
    ...
    blocked_connector_candidates: List[Dict[str, Any]] = []
    ...
    stats: Dict[str, Any] = {
        ...
        "rebuilt": 0,
        "reverted": 0,
        "blocked_connector_reconstruction": 0,
        "skipped": 0,
        "missing_anchor": 0,
        "geometry_before": {},
        "geometry_written": {},
        "examples": examples,
        "blocked_connector_candidates": blocked_connector_candidates,
        "errors": errors,
    }
        ...
        for conn in connections:
            ...
            if written_kind is None:
                ...
                blocked = {
                    ...
                    "reason": "straight_chord_fallback_disabled",
                }
                if len(blocked_connector_candidates) < int(max_examples):
                    blocked_connector_candidates.append(blocked)
                after_record = dict(before_record)
                after_record.update(
                    {
                        "action": "blocked_connector_reconstruction",
                        "code": "BLOCKED_CONNECTOR_RECONSTRUCTION",
                    }
                )
    ...
    stats["ok"] = bool(
        ...
        and int(stats["blocked_connector_reconstruction"]) == 0
    )
    return stats


def _risk_payload_from_rebuild(report: Dict[str, Any]) -> Dict[str, Any]:
    blocked = int(report.get("blocked_connector_reconstruction", 0) or 0)
    failed = (
        int(report.get("start_gap_over_threshold_after", 0) or 0)
        + int(report.get("end_gap_over_tolerance_after", 0) or 0)
        + blocked
    )
    return {
        ...
        "blocked_connector_reconstruction": blocked,
