...
try:
    ...
    def carla_safety_sanitize_xodr(xodr_path: str, out_path: str | None = None, **kwargs):
        ...
        try:
            laneoffset_max = float(
                kwargs.get("junction_laneoffset_abs_max", os.getenv("CARLA_SAFETY_JUNCTION_LANEOFFSET_MAX", "2.0")))
        except Exception:
            laneoffset_max = 2.0
        ...
        try:
            ...
            if re.search(r"\b(nan|inf|-inf)\b", txt, flags=re.IGNORECASE):
                ...
                _Res.findings.append(
                    {"code": "NAN_INF_DETECTED", "severity": "error", "message": "NaN/Inf tokens detected"})
                return _Res
        except Exception:
            pass
        ...
        for road in root.findall("road"):
            ...
            name = road.get("name", "")
            is_junction = (junction is not None and junction != "-1") or (
                name.startswith(":") if isinstance(name, str) else False)
            if not is_junction:
                continue
...
def _run_tile_worker(
    ...
    """
    Runs a tile worker in a subprocess.
    ...
    with log_path.open("w", encoding="utf-8") as f:
        try:
            ...
            f.write(f"\n[tile_qa_batch] unexpected error: {str(e)}\n")
            f.flush()
...
def main() -> int:
    ...
    for i, tile_name in enumerate(tiles):
        ...
        tile_path_for_worker = Path(getattr(safe_res, "out_path", str(tile_path))) if getattr(safe_res, "out_path",
                                                                                              None) else tile_path
