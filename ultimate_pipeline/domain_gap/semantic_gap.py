...
class SemanticGap:
    ...
    Measures:
      ...
      - lane marking density
      - road type length distributions

    All metrics are reported as:
      - absolute values
      - normalized deltas (scale-invariant)
    """
    ...
    def _total_road_length(root: ET.Element) -> float:
        return sum(
            SemanticGap._safe_float(r.get("length"))
    ...
    def compare(manual_xodr: str, auto_xodr: str) -> Dict[str, Any]:
        ...
        obj_delta = {
            t: obj_a.get(t, 0) - obj_m.get(t, 0)
            for t in all_obj_types
        }

        # -----------------------------
        # Road types (length-weighted)
        # -----------------------------
        ...
        rt_delta = {
            t: rt_a.get(t, 0.0) - rt_m.get(t, 0.0)
            for t in all_rtypes
        }
        ...
        rt_norm_delta = {
            t: (rt_a.get(t, 0.0) / total_len_a) -
               (rt_m.get(t, 0.0) / total_len_m)
            for t in all_rtypes
        }
        ...
        return {
            ...
            "road_types": {
                "manual_length": rt_m,
                "auto_length": rt_a,
                "delta_length": rt_delta,
                "delta_normalized": rt_norm_delta,
            },
            "traffic_lights": {
                "manual_count": tl_m,
                "auto_count": tl_a,
                "manual_density": tl_density_m,
                "auto_density": tl_density_a,
                "delta_density": tl_density_a - tl_density_m,
            },
            "lane_markings": {
                "manual_count": lm_m,
                "auto_count": lm_a,
                "manual_density": lm_density_m,
                "auto_density": lm_density_a,
                "delta_density": lm_density_a - lm_density_m,
            },
            "meta": {
                "manual_total_road_length": total_len_m,
                "auto_total_road_length": total_len_a,
                "normalized": True,
                "settings": {
                    "reference": getattr(SETTINGS, "PROJECT_NAME", "unknown"),
                },
            },
        }
