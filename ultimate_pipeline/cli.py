...
# =============================================================================
# RoadRunner Commands (optional backend, disabled by default)
# =============================================================================

@cli.group(hidden=True)
def roadrunner() -> None:
    """RoadRunner backend commands (disabled by default).

    Set UP_ENABLE_ROADRUNNER=1 to activate.
    Missing installation returns NOT_APPLICABLE.
    """
    pass


def _require_roadrunner_enabled() -> bool:
    """Return True only when the user has opted into RoadRunner commands."""
    return os.getenv("UP_ENABLE_ROADRUNNER") == "1"


@roadrunner.command("probe")
@click.option("--json", "as_json", is_flag=True, help="Output as JSON")
def rr_probe(as_json: bool) -> None:
    """Probe RoadRunner and MATLAB installation.

    Returns NOT_APPLICABLE when RoadRunner backend is disabled.
    """
    if not _require_roadrunner_enabled():
        click.echo("NOT_APPLICABLE: RoadRunner backend is disabled (set UP_ENABLE_ROADRUNNER=1)")
        sys.exit(0)

    try:
        from ultimate_pipeline.roadrunner.capability_probe import run_capability_probe
        report = run_capability_probe()
    except Exception as e:
        click.echo(f"BLOCKED: capability probe failed — {e}")
        sys.exit(1)
    ...
    if as_json:
        ...
        data = {
            ...
            "results": [
                {"name": r.name, "available": r.available, "detail": r.detail, "severity": r.severity}
                for r in report.results
            ],
        }
        click.echo(json.dumps(data, indent=2))
    else:
        click.echo(f"RoadRunner overall status: {report.overall_status}")
        for r in report.results:
...
@roadrunner.command("probe-installation")
@click.option("--json", "as_json", is_flag=True, help="Output as JSON")
def rr_probe_installation(as_json: bool) -> None:
    """Probe RoadRunner and MATLAB installation details.
    ...
    try:
        ...
        click.echo(f"BLOCKED: installation probe failed — {e}")
        sys.exit(1)
    ...
    if as_json:
        ...
        click.echo(f"RoadRunner executable: {report.roadrunner_executable or 'NOT FOUND'}")
        click.echo(f"MATLAB executable: {report.matlab_executable or 'NOT FOUND'}")
        click.echo(f"RoadRunner API available: {report.roadrunner_api_available}")
        click.echo(f"Automated Driving Toolbox: {report.automated_driving_toolbox}")
        click.echo(f"Scene Builder: {report.scene_builder}")
        if not report.roadrunner_executable:
            sys.exit(0)


@roadrunner.command("gate-status")
@click.option("--profile", default="structural_release", help="Gate matrix profile")
def rr_gate_status(profile: str) -> None:
    """Evaluate RoadRunner gate matrix for a profile.
    ...
    try:
        from ultimate_pipeline.roadrunner.gate_matrix import DEFAULT_GATE_MATRIX_MODEL
        from ultimate_pipeline.roadrunner.models import GateStatus, StageGate

        gates = [
            StageGate(gate_id="source_integrity", status=GateStatus.NOT_APPLICABLE),
            StageGate(gate_id="semantic_diff", status=GateStatus.NOT_APPLICABLE),
            StageGate(gate_id="mesh_xodr_alignment", status=GateStatus.NOT_APPLICABLE),
            StageGate(gate_id="artifact_hashes", status=GateStatus.NOT_APPLICABLE),
        ]
        decision = DEFAULT_GATE_MATRIX_MODEL.evaluate(profile, gates)
        click.echo(f"Profile: {profile}")
        click.echo(f"Release allowed: {decision.release_allowed}")
        if decision.missing_required_gates:
            click.echo(f"Missing required: {decision.missing_required_gates}")
        if decision.rejected_gates:
            click.echo(f"Rejected gates: {decision.rejected_gates}")
        if decision.illegal_not_applicable_gates:
            click.echo(f"Illegal NOT_APPLICABLE: {decision.illegal_not_applicable_gates}")
    except Exception as e:
        click.echo(f"BLOCKED: gate evaluation failed — {e}")
        sys.exit(1)


@roadrunner.command("version")
def rr_version() -> None:
    """Show RoadRunner integration module version.
    ...
    try:
        from ultimate_pipeline.roadrunner import __version__ as rr_version_str
    except ImportError:
        try:
            from importlib.metadata import version
            rr_version_str = version("ultimate_pipeline")
        except Exception:
            rr_version_str = "0.1.0"

    click.echo(f"RoadRunner integration module v{rr_version_str}")
