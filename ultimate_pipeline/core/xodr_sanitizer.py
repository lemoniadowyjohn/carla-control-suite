...
class XODRSanitizer:
    ...
    What it does:
      ...
      - Clamps laneSection.s values in (-1.0, 0.0) to 0.0 (float-noise from OSM→SUMO).
    ...
    # ---------- laneSection s epsilon clamp ----------

    @staticmethod
    def _fix_lane_section_s(road: ET.Element) -> int:
        """
        Clamp laneSection.s values that are slightly negative to 0.0.

        OSM→SUMO conversion occasionally produces laneSection s attributes
        with tiny negative values (e.g. -0.006) due to floating-point noise.
        The OpenDRIVE 1.4 spec requires s >= 0; CARLA and strict validators
        reject maps containing negative s values.

        This is an epsilon clamp only: values below -1.0 m are left untouched
        and reported as warnings, as they likely indicate a genuine structural
        issue rather than floating-point noise.
        """
        fixed = 0
        rid = road.get("id", "UNKNOWN")
        for ls in road.findall(".//lanes/laneSection"):
            ...
            if s_val <= -1.0:
                log.warning(
                    f"Road {rid}: laneSection s={s_val:.6f} is at or below -1.0 m — "
                    "skipping clamp (likely structural issue, not float noise)."
                )
                continue
            ls.set("s", "0.0")
            fixed += 1
            diff_log.add(
                "xodr_sanitizer",
                rid,
                {"fix": "lane_section_s_clamped", "old": raw_s, "new": "0.0"},
            )
        return fixed
    ...
    def sanitize_xodr(input_path: str, output_path: str, run_sumo: bool = False) -> bool:
        ...
        for road in root.findall("road"):
            ...
            sfix = XODRSanitizer._fix_lane_section_s(road)

            if wfix or gfix or sfix:
                log.debug(f"Road {rid}: fixed {wfix} widths, {gfix} geometries, {sfix} laneSection s.")

            total_width_fixes += wfix
            total_geom_fixes += gfix
            total_ls_s_fixes += sfix

        log.info(
            f"Lane width fixes: {total_width_fixes}, geometry fixes: {total_geom_fixes}, "
            f"laneSection s clamps: {total_ls_s_fixes}"
        )
