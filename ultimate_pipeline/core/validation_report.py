...
class ValidationReport:
    ...
    Fully compatible with:
      - an old pipeline (log_success / log_failure / log_skipped)
    ...
    def log_success(self, stage: str, msg: str = ""):
        self._add_stage_entry(stage, "success", msg)

    def log_failure(self, stage: str, reason: str):
        self._add_stage_entry(stage, "failure", reason)

    def log_skipped(self, stage: str):
        self._add_stage_entry(
            stage, "skipped", "stage disabled or not executed"
        )

    def _add_stage_entry(self, stage, status, details):
        ts = time.strftime("%Y-%m-%d %H:%M:%S")
        self.stage_entries.append(
            {"timestamp": ts, "stage": stage, "status": status, "details": details}
        )
        self._write()
    ...
    def record_failing_road(self, rid: str):
        self.data["failing_roads"].add(rid)
        self._write()
