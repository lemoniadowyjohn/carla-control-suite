...
def _drop_links_outside_tile(tile_root: ET.Element) -> dict:
    road_ids = _tile_road_ids(tile_root)
    parent_map = _build_parent_map(tile_root)

    dropped_items = []

    def _ancestor(el: ET.Element, tag: str) -> Optional[ET.Element]:
        cur = el
    ...
    for link in tile_root.findall(".//lane/link/*"):
        ...
        if road and road not in road_ids:
            ...
            if parent is not None:
                lane_el = parent_map.get(parent)
                ls_el = _ancestor(parent, "laneSection")
                road_el = _ancestor(parent, "road")
                lane_id = lane_el.get("id") if lane_el is not None else None
                road_id = road_el.get("id") if road_el is not None else None
                lane_section_index = None
                if road_el is not None and ls_el is not None:
                    try:
                        lss = road_el.findall("./lanes/laneSection")
                        lane_section_index = lss.index(ls_el)
                    except Exception:
                        lane_section_index = None
                ...
                dropped_items.append(
                    {
                        "road_id": road_id,
                        "lane_section": lane_section_index,
                        "lane_id": int(lane_id) if lane_id is not None else None,
                        "link_type": link.tag,
                        "target_lane_id": link.get("lane") or link.get("id"),
                    }
            )
                parent.remove(link)
    ...
    if dropped_lane_links["count"]:
        print(
            f"[TILER] Dropped {dropped_lane_links['count']} lane link(s) pointing outside tile "
            f"(roads: {roads})"
        )
    return dropped_lane_links
...
def _road_bounds(road: ET.Element) -> Tuple[float, float, float, float]:
    ...
    geometry_digest = hashlib.sha256(
        ET.tostring(plan_view if plan_view is not None else ET.Element("planView"), encoding="utf-8")
    ).hexdigest()
    cache_key = (rid, geometry_digest) if rid is not None else None
...
def _finalize_tile(
    ...
    *,
    ...
    buffer_m: float,
    analysis: dict,
    dropped_lane_links: Optional[dict] = None,
    ...
    if dropped_lane_links and int(dropped_lane_links.get("count", 0) or 0) > 0:
        health["dropped_lane_links"] = dropped_lane_links
...
class TileExtractor:
    ...
    def tile(
        ...
        *,
        ...
        progress_every = int(os.getenv("UP_TILE_PROGRESS_EVERY", "100") or "100")
        if progress_every > 0:
            print(f"[TILER] Roads: {len(roads)} (progress every {progress_every} tiles)")
        ...
        for idx, r in enumerate(roads, start=1):
            ...
            if progress_every > 0 and idx % progress_every == 0:
                print(f"[TILER] Scanned {idx}/{len(roads)} roads...")
        ...
        for ix, iy, x, y in _iter_tile_origins(min_x, max_x, min_y, max_y, tile_size):
            ...
            dropped_lane_links = None
            if not allow_outside:
                dropped_lane_links = _drop_links_outside_tile(tile_root)
            ...
            out_path, health = _finalize_tile(
                ...
                analysis=analysis,
                dropped_lane_links=dropped_lane_links,
            ...
            if progress_every > 0 and len(tiles) % progress_every == 0:
                print(f"[TILER] Wrote {len(tiles)} tiles...")
