...
def _build_adjacency(
    ...
    def _node_key(element_type: str | None, element_id: str) -> str:
        prefix = "junction" if element_type == "junction" else "road"
        return f"{prefix}:{element_id}"

    adj: Dict[str, set] = {}
    for road in roads:
        rid = road.get("id", "")
        node = _node_key("road", rid)
...
def quarantine_island_roads(
    ...
    # Road-only component sizes (exclude typed junction nodes from the count).
    road_components: List[set] = []
    for comp in components:
        comp_roads = {
            node.removeprefix("road:")
            for node in comp
            if node.startswith("road:")
        } & road_ids
        if comp_roads:
