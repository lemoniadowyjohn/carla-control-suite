...
def map_signature(world: carla.World) -> Dict[str, object]:
    ...
    sig_obj["sha256"] = hashlib.sha256(blob).hexdigest()
...
def main() -> None:
    ...
    for i in range(int(args.reps)):
        ...
        sigs.append(s)
        print(f"signature sha256={s['sha256']}  topology_edges={s['topology_edges']}  spawn_points={s['spawn_points']}")
        print(f"signature md5={s['md5']}")

    sha256s = [s["sha256"] for s in sigs]
