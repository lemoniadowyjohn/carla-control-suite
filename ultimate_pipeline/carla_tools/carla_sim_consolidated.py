...
# NOTE: Keep imports CARLA-free at module import time.
# The following helpers depend on CARLA, so we import them lazily inside
# CarlaSimulation.__init__.
...
class CarlaSimulation:
    ...
    def __init__(
        ...
        if not _CARLA_AVAILABLE:
            raise RuntimeError(
                ...
                "Install/activate CARLA PythonAPI before using CarlaSimulation."
            )
    ...
    def tick(self) -> None:
        ...
        try:
            ...
            print(f"[Sim] CARLA tick failed: {e} — stopping simulator")
            self.running = False
            return
