...
def restart_carla(
    ...
    """
    Retry CARLA startup until both RPC and streaming ports are open.
    """
    ...
    # Retry connection until both RPC and streaming ports are open
    t0 = time.time()
    for _ in range(max_tries):
        # Give CARLA time to start up
        time.sleep(wait_s)
        if _wait_for_ports(h, p, sp, timeout_s=wait_s):  # Custom helper to wait for both ports
            return True
        time.sleep(3)  # Allow some time before retrying

    # Final check after retries window
    if _port_open(h, p) and _port_open(h, sp):
        return True
...
def get_reliable_client() -> carla.Client:
    ...
    while attempts < SETTINGS.CARLA_CONNECT_RETRIES:
        ...
        # If RPC port open → check streaming and try connect
        if _port_open(host, port):
            # Check streaming port too. LibCarla crashes if this is refused later.
            if _port_open(host, streaming_port):
                print(f"📡 Ports {port} and {streaming_port} are open. Settling…")
                time.sleep(3)  # Give CARLA's internal streaming server a moment to fully bind
                # Check streaming port until it becomes stable
                retries = 10  # Try 10 times for better stability
                for attempt in range(retries):
                    if _port_open(host, streaming_port):
                        ...
                        print(
                            f"⏳ Streaming port {streaming_port} still closed. Waiting… Attempt {attempt + 1}/{retries}")
                ...
                print(f"⏳ RPC open, but streaming port {streaming_port} is still closed. Waiting…")
                time.sleep(2)
                if attempts > 5 and SETTINGS.CARLA_KILL_ON_FAIL:
                    print("⚠ Streaming port stuck closed → force restart.")
                    _kill_stale_processes()
                    _purge_temp_dirs()
