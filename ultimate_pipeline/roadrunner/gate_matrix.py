...
class GateMatrixProfile:
    """Required and optional gates for one release profile."""
...
class GateMatrixDecision:
    """Decision returned by gate matrix evaluation."""
...
class GateMatrix(SerializableContract):
    """Named gate matrix containing release profiles and gate policy."""
    ...
    def evaluate(self, profile: str, gates: Sequence[StageGate]) -> GateMatrixDecision:
        """Evaluate gates against this matrix."""
...
def evaluate_gate_matrix(
    ...
    *,
    ...
    """Evaluate gates and reject releases that violate profile policy.

    Required gates with FAIL or BLOCKED reject release. NOT_APPLICABLE is
    accepted only for gates listed as optional by the selected profile.
    """
...
def assert_release_allowed(profile: str, gates: Sequence[StageGate]) -> None:
    """Raise RoadRunnerGateError when the selected profile cannot release."""
