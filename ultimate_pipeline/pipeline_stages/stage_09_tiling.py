...
def _step9_tiling(self, final_out: str) -> Optional[str]:
    ...
    if (
        ...
        if rc != 0:
            # TIL-009: tile failures can NEVER be allowed through in release
            # mode.  The UP_ALLOW_TILE_QA_FAIL bypass is honored only outside
            # release profiles (and only when the settings gate allows it).
            release_mode = (
                getattr(self.settings, "THESIS_STRICT", False)
                or getattr(self.settings, "STRICT_QUALITY_GATES", False)
            ...
            if not release_mode and allow_skip and bypass_env:
                status_path = os.path.join(self.out_dir, "tile_qa_status.json")
                status = {
                    "status": "FAIL",
