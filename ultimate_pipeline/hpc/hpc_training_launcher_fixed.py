# hpc_training_launcher_fixed.py
import json
import os
import subprocess
...
class HPCTrainingLauncher:
    ...
    def launch_local_training(self, config_name):
        ...
        # Use the fixed trainer without Unicode
        cmd = [
            'python', 'thesis_perception_trainer_fixed.py',
        ...
        try:
            ...
            print("thesis_perception_trainer_fixed.py not found")
            return False
...
"""
    ...
    def check_hpc_readiness(self):
        """Check if system is ready for HPC"""
        ...
        # Check config files
        configs = ['osm_only', 'mixed_domains', 'all_osm']
        for config in configs:
            config_path = self.hpc_configs / f"domain_exp_{config}.json"
        ...
        # Check scripts
        for config in configs:
            script_path = self.hpc_configs / f"submit_{config}.sh"
        ...
        # Check data
        data_dirs = ['berlin', 'cologne', 'frankfurt', 'hamburg', 'munich', 'ingolstadt']
        for city in data_dirs:
            city_path = Path("thesis_data/datasets") / city
            if city_path.exists():
                print(f"✅ Data: {city}")
                checks.append(True)
            else:
        ...
        if all(checks):
            print("\n🎯 HPC SYSTEM READY!")
            print("You can submit jobs to HPC cluster")
            return True
        else:
...
def main():
    ...
    parser.add_argument('--check-readiness', action='store_true', help='Check HPC readiness')
