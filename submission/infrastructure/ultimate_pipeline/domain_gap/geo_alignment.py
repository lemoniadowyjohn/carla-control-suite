...
def _capture_elevation_profiles(root: ET.Element) -> List[Tuple[ET.Element, int, ET.Element]]:
    preserved: List[Tuple[ET.Element, int, ET.Element]] = []
    for road in root.findall(".//road"):
        profile = road.find("./elevationProfile")
        if profile is None:
            continue
        try:
            idx = list(road).index(profile)
        except ValueError:
            idx = len(list(road))
        preserved.append((road, idx, _clone_element(profile)))
    return preserved


def _restore_elevation_profiles(preserved: List[Tuple[ET.Element, int, ET.Element]]) -> None:
    for road, idx, profile in preserved:
        current = road.find("./elevationProfile")
        insert_at = idx
        if current is not None:
            try:
                insert_at = list(road).index(current)
            except ValueError:
                insert_at = idx
            road.remove(current)
        road.insert(min(insert_at, len(list(road))), profile)
...
class GeoAligner:
    ...
    def apply_to_xodr(
        ...
        root = tree.getroot()
        preserved_elevation_profiles = _capture_elevation_profiles(root)
        ...
        # Alignment is rigid in planView only; preserve original elevation polynomials verbatim.
        _restore_elevation_profiles(preserved_elevation_profiles)
