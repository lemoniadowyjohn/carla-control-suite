...
def _write_xodr_with_elevation(
    path: Path,
    *,
    x1: float,
    y1: float,
...
<OpenDRIVE>
  ...
  <road name="TestRoad" length="100.0" id="1" junction="-1">
    ...
    <elevationProfile>
      <elevation s="0.0" a="{elevation_a}" b="{elevation_b}" c="0.0" d="0.0"/>
    </elevationProfile>
    <lanes>
      <laneSection s="0.0">
        <center><lane id="0" type="none" level="false"/></center>
...
class TestApplyToXodr:
    ...
    def test_apply_to_xodr_preserves_elevation_profile_coefficients(self, tmp_path: Path) -> None:
        in_xodr = tmp_path / "input_elevation.xodr"
        out_xodr = tmp_path / "aligned_elevation.xodr"
        _write_xodr_with_elevation(
            in_xodr,
            x1=0.0,
            y1=0.0,
            x2=50.0,
        ...
        transform = {"scale": 1.0, "cos": 0.0, "sin": 1.0, "tx": 10.0, "ty": 20.0}
        result = GeoAligner.apply_to_xodr(str(in_xodr), str(out_xodr), transform)

        assert result is True
        tree = ET.parse(out_xodr)
        elevation = tree.getroot().find("./road/elevationProfile/elevation")
        assert elevation is not None
        assert float(elevation.get("a", "nan")) == pytest.approx(365.0, abs=0.001)
        assert float(elevation.get("b", "nan")) == pytest.approx(0.001, abs=1e-6)
