...
class ContractModel(BaseModel):
    """Shared model config for strict contract validation."""

    model_config = ConfigDict(extra="forbid")


class BboxContract(ContractModel):
...
class SensorRigContract(ContractModel):
...
class DeterminismContract(ContractModel):
...
class TilePairingContract(ContractModel):
...
class EnvironmentContract(ContractModel):
    ...
    optional: List[str] = Field(default_factory=lambda: [
        ...
        "UP_REQUIRE_CARLA_FOR_THESIS",
        "UP_TILE_CORRESPONDENCE_CSV",
        "UP_ALLOW_IDENTITY_ALIGNMENT",
        "UP_ALLOW_EMPTY_CORRESPONDENCE",
        "UP_ALLOW_ALIGNMENT_WITH_HEADER_OFFSET",
        "UP_THESIS_STRICT",
        "UP_PREANCHOR_INPUT_XODR",
        "UP_WRITE_DRIVABILITY_SANITY",
        "UP_REPAIR_ROAD_LINK_ENDPOINTS",
        "UP_REPAIR_ROAD_LINK_TOL_M",
        "UP_AUTO_EXPAND_DEM",
        "UP_DEM_EXPAND_MARGIN_DEG",
        "UP_DEM_EXPANDED_DIR",
        "UP_ELEVATION_END_SAMPLE_EPS_M",
    ])


class LockPolicyStrictBehavior(ContractModel):
    """Strict lock-behavior semantics."""
...
class LockPolicyContract(ContractModel):
    """Lock contract for single-writer enforcement."""

    required: bool = True
    enforce_exclusive: bool = True
    lease_timeout_s: int = 3600
    lock_file: str = ".agent_lock.json"
...
class EntrypointPolicyContract(ContractModel):
    """Canonical and allowed direct entrypoints policy."""

    canonical_entrypoint: str = "python -m ultimate_pipeline.cli"
    allow_direct_entrypoints_scenario_b: bool = True
    allowed_direct_entrypoints: List[str] = Field(default_factory=lambda: [
        "python -m ultimate_pipeline.run_determinism_audit",
        "python -m ultimate_pipeline.run_full_domain_gap",
...
class EvidencePolicyContract(ContractModel):
    """Stable evidence path policy for thesis-bound outputs."""

    forbid_tmp_evidence_in_submission: bool = True
    stable_prefixes: List[str] = Field(default_factory=lambda: [
    ...
    allow_tmp_paths: bool = False
    required_manifest: bool = True
    stable_roots: List[str] = Field(default_factory=lambda: [
        "artifacts/",
        "thesis_results/",
        "docs/submission/",
    ])
...
class AgentSyncContract(ContractModel):
    """
    ...
    entrypoint_policy: EntrypointPolicyContract = Field(default_factory=EntrypointPolicyContract)
    evidence_policy: EvidencePolicyContract = Field(default_factory=EvidencePolicyContract)
    thesis_strict_artifacts: List[str] = Field(default_factory=lambda: [
        "audit_summary.json",
        "crs_comparability.json",
        "preanchor_applied.txt",
        "gps_qa_crop_disabled_strict.txt",
        "carla_drivability_sanity.json",
    ])
    manual_map_fallback: List[str] = Field(default_factory=lambda: [
...
def validate_agent_sync(
    ...
    try:
        ...
        if strict:
            ...
            if not contract.lock_policy.required:
                result["errors"].append("Strict mode requires lock_policy.required=true")
            if contract.evidence_policy.allow_tmp_paths:
                result["errors"].append("Strict mode requires evidence_policy.allow_tmp_paths=false")
