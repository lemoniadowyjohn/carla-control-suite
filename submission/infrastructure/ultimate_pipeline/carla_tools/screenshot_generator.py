...
"""NOTE ABOUT IMPORT SAFETY

This module is imported transitively by parts of the pipeline and by unit tests.
The CARLA PythonAPI is *not* available in many environments (CI, doc builds,
local development without CARLA installed).

Therefore we never raise at import time if `carla` is missing. Instead, we
defer the error to *runtime* when a ScreenshotGenerator is actually
constructed/used.
"""
    ...
    def __init__(self, client: "carla.Client", out_dir: str):
        if not _CARLA_AVAILABLE:
            raise RuntimeError(
                ...
                "Install/activate CARLA PythonAPI before using ScreenshotGenerator."
            )
        self.client = client
