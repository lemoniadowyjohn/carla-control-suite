...
class TileMetadata:
    ...
    def write_manifest(
        ...
        *,
        proj4_norm: str,
        proj4_params_complete: bool | None = None,
        tile_size_m: float,
        buffer_m: float,
        frame_method: str = "native",
        ...
        min_x = None
        min_y = None
        for name in sorted(k for k in data.keys() if isinstance(k, str) and not k.startswith("_")):
            v = data.get(name)
            ...
            try:
                bx0 = float(bbox.get("min_x"))
                by0 = float(bbox.get("min_y"))
                bx1 = float(bbox.get("max_x"))
                by1 = float(bbox.get("max_y"))
            except Exception:
                continue
            ...
            tiles.append({
                ...
                "bbox": {"min_x": bx0, "min_y": by0, "max_x": bx1, "max_y": by1},
                "center": {
                    "x": float(center.get("x", (bx0 + bx1) * 0.5)),
                    "y": float(center.get("y", (by0 + by1) * 0.5)),
                },
            })

        resolved_origin_x = origin_x
        resolved_origin_y = origin_y
        if resolved_origin_x is None and isinstance(transform, dict):
            try:
                if transform.get("origin_x") is not None:
                    resolved_origin_x = float(transform.get("origin_x"))
            except Exception:
                resolved_origin_x = None
        if resolved_origin_y is None and isinstance(transform, dict):
            try:
                if transform.get("origin_y") is not None:
                    resolved_origin_y = float(transform.get("origin_y"))
            except Exception:
                resolved_origin_y = None
        if resolved_origin_x is None:
            resolved_origin_x = float(min_x or 0.0)
        if resolved_origin_y is None:
            resolved_origin_y = float(min_y or 0.0)

        manifest = {
            "schema_version": "1",
            "proj4_norm": str(proj4_norm or ""),
            "proj4_params_complete": proj4_params_complete if proj4_params_complete is not None else None,
            "origin_x": float(resolved_origin_x),
            "origin_y": float(resolved_origin_y),
