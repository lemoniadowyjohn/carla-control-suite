...
class SegmentationDatasetGenerator:
    ...
    def _setup(self) -> None:
        ...
        # Use the current DominikSensorSetup API: __init__ loads calib JSON into
        # _calib; use _parse_camera_transform for thesis-correct CTV handling.
        setup = DominikSensorSetup(self.calib_path.as_posix())
        cameras_data = setup._calib.get("cameras", {})
        cam_names = list(cameras_data.keys())

        selected = cam_names if self.camera == "all" else [self.camera]
        for cam_name in selected:
            cam_data = cameras_data[cam_name]
            tf_dict = setup._parse_camera_transform(cam_data)
            tf = carla.Transform(
            ...
            size = cam_data.get("image_size", [1920, 1080])
            w, h = int(size[0]), int(size[1])
            K = cam_data.get("K_undistortion") or cam_data.get("K")
            # Derive FOV from horizontal focal length; fall back to 90° if missing.
            fov = (
                2.0 * math.degrees(math.atan(float(w) / (2.0 * float(K[0][0]))))
                if K
                else 90.0
            )
...
def parse_args():
    ...
    ap.add_argument("--camera", default="all", help="camera name or 'all'")
