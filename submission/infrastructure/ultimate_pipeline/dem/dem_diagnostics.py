    ...
    def summarize(dem_path: str) -> dict:
        ...
        try:
            with rasterio.open(dem_path) as ds:  # type: ignore[union-attr]
                ...
                # Read band 1 as masked array to exclude nodata sentinel values
                band = ds.read(1, masked=True)
                valid_count = int(band.count()) if hasattr(band, 'count') else None
                min_val = float(band.min()) if valid_count and valid_count > 0 else None
                max_val = float(band.max()) if valid_count and valid_count > 0 else None
