...
\begin{table}[ht]
\centering
\caption[Structural overview of the three CARLA environments (auto-gen., Grid0821, Grid0828).]{Structural overview of the three CARLA environments used in this thesis. Road counts and lengths for the manual maps are computed from the OpenDRIVE XML; spawn points are confirmed by CARLA stage-probe artifacts. Auto-generated road count and total road length are anchored to the authoritative run\_11 artifact, while the auto junction count comes from the governed connectivity companion artifact family and the auto curvature standard deviation comes from the corrected support measurement on the authoritative manual-vs-contract-run pair.}
...
CARLA spawn points & 8{,}267--8{,}535 (load probes) & 11,634 & 11,634 \\
Mean lane-section entries / road & 1.14 & \multicolumn{2}{c}{43.9 (Grid0828)} \\
Curvature std (m$^{-1}$) & 0.865 & \multicolumn{2}{c}{0.055 (Grid0828)} \\
RQ2 structural baseline & Yes (run\_11) & --- & Yes (primary) \\
Elevation (run\_11 authority) & planar$^\dagger$ & planar & planar \\
\hline
\end{tabular}
\par\smallskip
{\footnotesize $^\dagger$\,The run\_11 structural baseline is planar (no DEM). The promoted final contract XODR (\texttt{08\_final\_structural\_gap.xodr}) derives from the supplementary elevation run and carries non-planar elevation (362--392\,m); it is retained as supplementary evidence and does not alter the run\_11 structural measurements.}
\end{table}
...
Therefore, \emph{valid OpenDRIVE} (XML/schema validity) is not necessarily \emph{structurally validated}. The pipeline enforces stricter constraints aligned with CARLA’s internal assumptions.
...
In CARLA, sensors are attached to actors using a \texttt{carla.}\allowbreak\texttt{Transform} defined relative to the parent actor (here: the ego vehicle). \cite{carla_docs_sensors}
...
\begin{itemize}
\item \textbf{Topological determinism:} the same input and settings must yield invariant topological signatures (road count, junction count, lane distributions); byte-level output identity is not required and is not achieved in practice (RQ1 verdict: topologically \texttt{DETERMINISTIC}, byte-level \texttt{NONDETERMINISTIC}).
