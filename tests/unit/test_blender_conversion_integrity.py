...
class FakeBlenderRunner(BlenderRunner):
    ...
    def __init__(self, *, inject_stdout: str = "CONVERSION_OK", inject_stderr: str = "",
                 ...
                 inject_blender_ok: bool = True,
                 inject_blender_ver: str = "Blender 4.3.0",
                 **kwargs):
        ...
        self._inject_blender_ok = inject_blender_ok
        self._inject_blender_ver = inject_blender_ver

    def _verify_fbx_container(self, path: Path) -> Dict[str, Any]:
        """Fake host-side FBX container validation: matches real _verify_fbx_container behavior."""
        if not path.exists():
