...
def test_connector_snap_is_enabled_by_default(monkeypatch, tmp_path):
    """Settings-audit decision (2026-09-12): default ON, not off. Full-map
    verification on the pinned map-of-record showed 4125/22589 connectors
    snapped, zero structural regressions -- see CONNECTOR_SNAP_WIRING_EVIDENCE.json.
    An unset env var must therefore still run the snap, not silently skip it."""
...
def test_connector_snap_disabled_via_explicit_opt_out(monkeypatch, tmp_path):
    stage_instance = _Stage(tmp_path)
    monkeypatch.setenv("UP_ENABLE_JUNCTION_CONNECTOR_SNAP", "0")
    monkeypatch.setattr(
