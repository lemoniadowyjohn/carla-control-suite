...
| RQ4 — Structural variability / latent representation | The thesis fixed GNN representation collapse using NT-Xent and obtained statistically supported latent separation, including a K=1000 permutation test with p<0.001. | **AUTHORITATIVE (leak-free, 2026-09-24) — supersedes the 2026-09-01 leaky number.** A real 5-seed GNN ensemble (same design: 562 union tiles, 50 epochs, batch 16, lr 1e-4, CPU, seeds 42-46) retrained with the GAP-010 leak-exclusion mechanism active reports **cosine_distance mean 0.9207 (95% bootstrap CI [0.845, 0.976])**, CI excludes zero across all 5 seeds. Both a pre-training and a post-hoc per-checkpoint leakage audit PASS (`eval_pair_hash_overlap: []` for every seed) — see `reports/production_readiness/20260924T000000Z_GAP010_RQ4_LEAKFREE_RETRAIN/RESULTS.md`. **Prior (leaky) number, kept for comparison, NOT for citation**: cosine_distance mean 0.6434 (95% CI [0.616, 0.676]), computed 2026-09-01 before the leakage was discovered. The direction of the finding holds in both; the leak-free result shows a materially LARGER cosine_distance (stronger apparent latent separation) than the leaky one — the opposite of a naive "leakage inflates the result" expectation, plausibly because the leaked tile's shared content was pulling the two domains' learned representations closer together during training. The 95% CI is still a bootstrap over only **n=5 seeds** (fragile tails — the thesis's K=1000 permutation p<0.001 remains the *primary* statistical support), and the metric remains an in-sample union-domain latent-separation **diagnostic**, not held-out accuracy or transfer. Explicit domain-randomization wiring is confirmed, but governed natural-vs-explicit DR experiments remain a separate extension. | The new contribution is the post-thesis multi-seed union-domain robustness layer (`C18_GNN_LATENT_GAP` → `C21_GNN_AUTHORITATIVE` → 2026-09-24 leak-free retrain), not the entire RQ4 result. **History**: numbers changed materially on 2026-09-01 (graph-construction self-loop bug fix), then again on 2026-09-24 (GAP-010 leakage fix + real retrain, both pre-training and post-hoc leakage audits PASS on the actual trained checkpoints). A real code gap was also found and fixed during the retrain: GAP-010's original exclusion fix never reached `run_gnn_pipeline.py`, the actual entrypoint the C21 protocol calls — now fixed. |
